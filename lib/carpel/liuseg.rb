...
class LiuSegmenter
  ...
  def prior_k k
    k > 0 ? ((1 - @null_prior) / @maxk) : @null_prior
  end
  ...
  def prob_R_given_k k
    ...
    (0...@seq.length - k).each do |i|
      ...
      left = lowerk[0][i];
      # right side is lookup up for k=1
      right = onek[i+1][@seq.length-1];
